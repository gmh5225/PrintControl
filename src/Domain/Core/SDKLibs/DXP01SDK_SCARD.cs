...
namespace CaasId.src.Domain.Core.SDKLibs
{
    ...
    public class SCard
    {
            ...
            ref uint protocol)
        {
            ...
            try
            {
                var initializationString = StringFromDisposition(initialization);
                var connectionTypeString = StringFromConnectionType(connectionType);
        ...
        // SCard::SCardControl()
        //
        // wrapper for the win32 ::SCardControl() function.
        //
        // see https://docs.microsoft.com/en-us/windows/win32/api/winscard/nf-winscard-scardcontrol
        //
        ...
        ////////////////////////////////////////////////////////////////////////////////
        public long SCardControl(byte[] sendBytes, ref byte[] receivedBytes)
        {
            long scardResult = (long)scard_error.SCARD_F_UNKNOWN_ERROR;

            if (sendBytes.Length == 0)
            {
                return (long)scard_error.SCARD_S_SUCCESS;
            }
            ...
            try
            {
                ...
                var xmlFormat = strings.SMARTCARD_CONTROL_XML;
                var input = string.Format(xmlFormat, base64String);
                var resultXml = _bidiSpl.SetPrinterData(strings.SMARTCARD_CONTROL, input);
                var smartcardResponseValues = Util.ParseSmartcardResponseXML(resultXml);
        ...
        public long SCardDisConnect(int disposition)
        {
            ...
            try
            {
                ...
                var dispositionString = StringFromDisposition(disposition);
